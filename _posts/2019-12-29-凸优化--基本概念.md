...
**仿射集合 (Affine set)**：如果通过集合 $C\subseteq \R^n$ 中任意两个不同点的直线仍然在集合 $C$ 中，那么称集合 $C$ 是仿射的。是仿射 $C\subseteq \R^n$ 的等价于：对于任意 $x_1, x_2 \in C$  及 $\theta \in \R $  有 $$\theta x_1+(1-\theta)x_2 \in C$$
...
**超平面**：超平面是具有 $\{x \vert \alpha^T x = b\}$ 形式的集合 , 其中 $\alpha \in \R^n, \alpha \neq 0, b\in \R$。超平面是仿射的 (也是凸的)。
...
+ 仿射函数：如果函数 $f: \R^n → \R^m$ 是一个线性函数和一个常数的和，即具有 $f(x) = Ax + b$ 的形式，则它是仿射的，其中 $A\in \R^{m\times n}，b \in \R^m$。假设 $S\in\R^n$ 是凸的，是仿射函 $f: \R^n → \R^m$ 数， $S$ 在 f 下的象是凸的。如果 $f: \R^m → \R^n$  是仿射函数，那么 S 在 f 下的原象是凸的。

+ 透视函数：$ P: \R^{n+1} → \R^n, P(z, t) = z/t $为透视函数，其定义域为 $\mathbf{dom} P = R^n \times R_{++}$, 其中 $R_{++} = \{x\in \R \vert x > 0\}$

+ 线性分式函数：线性分式函数由透视函数和仿射函数复合而成。设 $g: \R^n → \R^{m+1} $ 是仿射的，即 $  g(x) = \left[  \begin{array}{}  A  \\c^T \end{array}\right]x + \left[ \begin{array}{} b\\d\end{array}\right]$, 其中  $A\in \R^{m\times n}，b \in \R^m, c\in \R^n, d\in\R$。则由 $f =P\circ g$ 给出的函数 

  $$
  f: \R^n → \R^m: f(x) = \frac{Ax + b)}{c^Tx + d}, \mathbf{dom}f=\{x\vert c^Tx+d>O\}
  $$
  
  称为线性分式(或投射)函数。如果$ c = 0, d > 0$，则 $f$ 的定义域为 $\R^n$ ， 并且 $f$ 是仿射函数。可以将仿射和线性函数视为特殊的线性分式函数。

  线性分式函数可以表示为 $f = \cal P^{-1}(Q\cal P(x))$  从 $x\in  \mathbf{dom}f$ 出发，可以得到 $\R^{n+1}$ 空间的 一条射线 $\cal P(x)$。将线性变换矩阵 $Q$ 作用于这条射线，就可以得到另一条射线 $Q\cal P(x)$。因 为  $x\in  \mathbf{dom}f$ . 这条射线的最后一个分量为正,可以通过逆投射变换恢复出 $f(x)$。 
...
函数 $f: \R^n → \R$ 是凸的，如果 $\mathbf{dom}f$  是凸集，且对于任意$x, y \in \mathbf{dom}f$ 和任意 $0\le \ \theta\le 1$，有
...
这个性质容许我们通过将函数限制在直线上来判断其是否是凸函数。即，即使 $f: \R^n → \R$，我们也可以通过判断  $g: \R → \R$ 来判断它的凹凸性。
...
对于 $\R$ 上的函数，上式可以简化为一个简单的条件 $f''\ge  0$,  ($\mathbf{dom}f$ 是凸的，即一 个区间)

##### **下水平集**(sublevel set)

函数  $f: \R^n → \R$ 的 $\alpha -$下水平集定义为
...
函数 $f: \R^n → \R$  的图像定义为 $\{(x,f(x))\vert x\in \mathbf{dom}f\}$， 它是 $\R^{n+1}$空间的一个子集。函数   $f: \R^n → \R$   的上境图定义为 
...
+ **复合仿射映射**: 设函数 $f: \R^n → \R, A\in R^{n\times m}, b\in \R^n$， 定义 $g: \R^m → \R: g(x) = f(Ax+b)$, 其中 $\mathbf{dom} g = \{x\vert Ax+b\in \mathbf{dom} f\}$。若函数 $f$ 是凸函数，则函数 $g$ 是凸函数:如果函数 $f$ 是凹函数，那么函数 $g$ 是凹函数。
...
+ **透视函数**：给定函数 $f: \R^n → \R$ ，则 f 的透视函数 $g: \R^{n+1} → \R$  定义为$g(x,t) = tf(x/t)$, 其定义域为 $\mathbf{dom}g = \{(x,t)\vert x/t\in \mathbf{dom}f, t>0\} $。透视运算是保凸运算: 如果函数 $f$ 是凸函数，则其透视函数 $g$ 也是凸函数。类似地，若 $f$ 是凹函数，则 $g$ 亦是凹函数。
...
<img src="C:\Users\13353\OneDrive\note\JoeyLian.github.io\_posts\images\image-20191229191335174.png" alt="image-20191229191335174" style="zoom: 80%;" />
